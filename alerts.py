...
def _citizens_in_radius(area_lat, area_lng, radius_km):
    """
    Return citizens whose registered location falls within radius_km of the
    given alert coordinates, using geopy's geodesic distance calculation.

    If the alert has no coordinates, or a citizen hasn't set a location,
    we fall back to notifying that citizen anyway — geofencing only narrows
    the audience when we actually have the data to do so safely; it should
    never silently leave people unnotified due to missing data.
    """
    ...
    in_range = []
    alert_point = (area_lat, area_lng)
    for citizen in citizens:
        if citizen.latitude is None or citizen.longitude is None:
            in_range.append(citizen)  # no location on file -> notify anyway
            continue
        distance_km = geodesic(alert_point, (citizen.latitude, citizen.longitude)).km
        if distance_km <= radius_km:
            in_range.append(citizen)
    return in_range
...
def create_alert():
    ...
    try:
        ...
        citizens = _citizens_in_radius(alert.area_latitude, alert.area_longitude, alert.area_radius)
        for citizen in citizens:
...
def update_alert(alert_id):
    ...
    db.session.commit()
    log_activity(int(get_jwt_identity()), 'alert_updated', f'Alert #{alert.id} updated')
